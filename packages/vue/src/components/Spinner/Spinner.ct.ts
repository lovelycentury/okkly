...
test.describe("Screenshot tests", () => {
  executeMatrixScreenshotTest({
    ...
    component: Spinner,
    args: (column, row) => ({ props: { color: column, size: row } as never }),
  });
  ...
  executeMatrixScreenshotTest({
    ...
    fastNoIsolation: true,
    component: Spinner,
    args: (column, row) => ({ props: { thickness: Number(column), size: row } as never }),
  });
});
...
test("should apply size modifiers only for non-medium sizes", async ({ mount }) => {
  // ARRANGE
  const component = await mount(Spinner, { props: { size: "small" } });
...
test("should apply color modifiers only for non-primary colors", async ({ mount }) => {
  // ARRANGE
  const component = await mount(Spinner, { props: { color: "dante" } });
