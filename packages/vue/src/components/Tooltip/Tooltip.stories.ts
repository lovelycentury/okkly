 ...
 * `leaveDelay`, a named `open` model with `defaultOpen`, `disableHoverListener`,
...
const meta: Meta<TooltipProps> = {
  ...
  render: (args) => ({
    ...
    template: `
      <div :style="surface">
        <Tooltip v-bind="args">
          <Button size="small" variant="secondary">Hover me</Button>
        </Tooltip>
      </div>`,
  }),
};
...
const iconButtonStyle =
  "display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; border: var(--okkly-1px-in-rem) solid var(--okkly-border-subtle); border-radius: 10px; background: var(--okkly-bg-surface); color: var(--okkly-text-secondary); cursor: pointer";
...
export const NamingIconButtons: Story = {
  ...
  render: () => ({
    components: { Tooltip },
    setup: () => ({ surface, captionStyle, iconButtonStyle, iconSettings, iconInfo, iconTrash }),
    template: `
      <div :style="surface">
        <Tooltip title="Settings">
          <button type="button" :style="iconButtonStyle" aria-hidden="false"><span v-html="iconSettings" /></button>
        </Tooltip>
        <Tooltip title="About this release">
          <button type="button" :style="iconButtonStyle"><span v-html="iconInfo" /></button>
        </Tooltip>
        <Tooltip title="Delete — this cannot be undone">
          <button type="button" :style="iconButtonStyle"><span v-html="iconTrash" /></button>
        </Tooltip>
        <p :style="captionStyle">Tab through them: each one is announced by its tooltip.</p>
      </div>`,
  }),
};
...
export const Placements: Story = {
  render: () => ({
    ...
    template: `
      <div :style="{ ...surface, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', width: '560px' }">
        <Tooltip v-for="placement in placements" :key="placement" :title="placement" :placement="placement">
          <Button size="small" variant="ghost">{{ placement }}</Button>
        </Tooltip>
      </div>`,
  }),
};
...
export const NamingVersusDescribing: Story = {
  ...
  render: () => ({
    components: { Tooltip, Button },
    setup: () => ({ surface, captionStyle, iconButtonStyle, iconSettings, iconInfo }),
    template: `
      <div :style="surface">
        <Tooltip title="Settings">
          <button type="button" :style="iconButtonStyle"><span v-html="iconSettings" /></button>
        </Tooltip>
        <Tooltip title="Makes your edits visible to everyone">
          <Button size="small" variant="secondary">Publish</Button>
        </Tooltip>
        <Tooltip title="Described, not named" describe-child>
          <button type="button" :style="iconButtonStyle"><span v-html="iconInfo" /></button>
        </Tooltip>
    ...
    template: `
      <div :style="surface">
        <Tooltip title="No arrow">
          <Button size="small" variant="secondary">Plain</Button>
        </Tooltip>
        <Tooltip title="With an arrow" arrow>
          <Button size="small" variant="secondary">Arrow</Button>
        </Tooltip>
      </div>`,
  }),
};
...
export const Delays: Story = {
  render: () => ({
    ...
    template: `
      <div :style="surface">
        <Tooltip title="No delay at all" :enter-delay="0">
          <Button size="small" variant="ghost">0ms</Button>
        </Tooltip>
        <Tooltip title="The default" :enter-delay="100">
          <Button size="small" variant="ghost">100ms</Button>
        </Tooltip>
        <Tooltip title="Deliberate" :enter-delay="600">
          <Button size="small" variant="ghost">600ms</Button>
        </Tooltip>
        <Tooltip title="Lingers after you leave" :enter-delay="0" :leave-delay="800">
          <Button size="small" variant="ghost">leaveDelay 800ms</Button>
        </Tooltip>
        <p :style="captionStyle">Sweep the pointer across all four to feel the difference.</p>
      </div>`,
  }),
};
  ...
  render: () => ({
    ...
    template: `
      <div :style="surface">
        <Tooltip>
          <template #title>
            <span>Not reachable — try to move onto this <a href="#nope" style="color: var(--okkly-accent-primary)">link</a></span>
          </template>
          <Button size="small" variant="ghost">Plain</Button>
        </Tooltip>
        <Tooltip interactive :leave-delay="150">
          <template #title>
            <span>Reachable — this <a href="#yes" style="color: var(--okkly-accent-primary)">link</a> can be clicked</span>
          </template>
          <Button size="small" variant="ghost">Interactive</Button>
        </Tooltip>
      </div>`,
  }),
};
...
/**
 * Pass `open` (as `v-model:open`) and the tooltip is yours to drive — for an
 * onboarding hint, a validation message, or anything that should appear without
 * the pointer. Listen for `@open`/`@close` to keep your state in step with the
 * hover and focus the component still detects.
 */
export const Controlled: Story = {
  render: () => ({
    ...
    setup() {
      const open = ref(true);
      return { surface, captionStyle, open };
    },
    template: `
      <div :style="surface">
        <Tooltip title="Held open from the outside" v-model:open="open" arrow>
          <Button size="small" variant="secondary">The target</Button>
        </Tooltip>
        <Button size="small" variant="ghost" @click="open = !open">{{ open ? "Hide it" : "Show it" }}</Button>
        <p :style="captionStyle">Hover does nothing here — open is the only thing deciding.</p>
      </div>`,
  }),
};
    ...
    template: `
      <div :style="surface">
        <Tooltip title="Hover and focus both">
          <Button size="small" variant="ghost">Both</Button>
        </Tooltip>
        <Tooltip title="Focus only" disable-hover-listener>
          <Button size="small" variant="ghost">Focus only — Tab to it</Button>
        </Tooltip>
        <Tooltip title="Hover only" disable-focus-listener>
          <Button size="small" variant="ghost">Hover only</Button>
        </Tooltip>
      </div>`,
  }),
};
      ...
      <div :style="surface">
        <Tooltip title="Add a title before you can publish">
          <span style="display: inline-flex" tabindex="0">
            <Button size="small" disabled>Publish</Button>
          </span>
        </Tooltip>
        <Tooltip title="This one hears nothing — the button swallows the events">
          <Button size="small" disabled>Publish (no wrapper)</Button>
        </Tooltip>
        <p :style="captionStyle">Only the first one responds.</p>
      </div>`,
  }),
};
