...
import CalendarFixture from "../../playwright/fixtures/CalendarFixture.vue";
...
/**
 * Mirrors `calendarToneStyle` from Calendar.vue. Spelled out here because a
 * component test's non-component imports are evaluated in Node, where the
 * component's SCSS import cannot resolve.
 */
const toneStyle = (tone: CalendarTone): Record<string, string> | undefined =>
  tone === "primary" ? undefined : { "--okkly-calendar-tone": `var(--okkly-accent-${tone})` };
...
test.describe("Screenshot tests", () => {
      ...
      row === "range"
        ? {
            props: {
              mode: "range",
              month: JANUARY_2024,
              modelValue: [new Date(2024, 0, 10), new Date(2024, 0, 18)],
  ...
  executeMatrixScreenshotTest({
    ...
    args: () => ({
      props: { month: JANUARY_2024, modelValue: new Date(2024, 0, 10) } as never,
    }),
  });
...
test("should render the visible month's title", async ({ mount }) => {
  // ARRANGE
  const component = await mount(Calendar, { props: { month: JANUARY_2024 } });
...
test("should retint from the style prop rather than a color modifier", async ({ mount }) => {
  // ARRANGE
  const tinted = await mount(Calendar, {
    props: { month: JANUARY_2024, style: toneStyle("dante") } as never,
  });

  // ASSERT
  expect(
    await tinted.evaluate((element) => element.style.getPropertyValue("--okkly-calendar-tone")),
  ).toBe("var(--okkly-accent-dante)");

  // ARRANGE — a second mount rather than `component.update()`: Playwright's
  // Vue CT harness updates props by mutating the component instance's props
  // directly instead of through a genuine parent re-render, which doesn't
  // reliably clear a fallthrough `style` key the way a real reactive update
  // does — the default tone sets nothing at all.
  const untinted = await mount(Calendar, {
    props: { month: JANUARY_2024, style: toneStyle("primary") } as never,
  });

  // ASSERT
  expect(
    await untinted.evaluate((element) => element.style.getPropertyValue("--okkly-calendar-tone")),
  ).toBe("");
});
...
test("should reorder weekday labels for a Sunday start", async ({ mount }) => {
  ...
  const component = await mount(Calendar, {
    props: { month: JANUARY_2024, weekStart: "sun" },
  });
...
test("should mark the selected date and report the clicked one", async ({ mount }) => {
  ...
  const component = await mount(Calendar, {
    props: { month: JANUARY_2024, modelValue: new Date(2024, 0, 10) } as never,
    on: { "update:modelValue": (value: unknown) => picked.push(value as Date) },
  });
...
test("should disable dates outside min/max", async ({ mount }) => {
  ...
  // ARRANGE
  const component = await mount(Calendar, {
    props: { month: JANUARY_2024, min: new Date(2024, 0, 5), max: new Date(2024, 0, 20) } as never,
    on: { "update:modelValue": (value: unknown) => picked.push(value as Date) },
  });
...
test.describe("today", () => {
  ...
  test("should keep the today modifier on a day that is also selected", async ({ mount, page }) => {
    ...
    await page.clock.setFixedTime(new Date(2024, 0, 17, 12));
    const component = await mount(Calendar, {
      props: { month: JANUARY_2024, modelValue: new Date(2024, 0, 17) } as never,
    });
    const today = component.getByRole("button", { name: "17", exact: true });
...
test.describe("month navigation", () => {
  test("should move month and report it via update:month", async ({ mount }) => {
    const months: Date[] = [];

    // ARRANGE — CalendarFixture keeps `month` genuinely controlled (see its
    // own comment): the prop stays pinned at January 2024 the whole test, so
    // both clicks compute fresh from it rather than from wherever the last
    // click left off.
    const component = await mount(CalendarFixture, {
      props: { month: JANUARY_2024 },
      on: { "month-change": (value: unknown) => months.push(value as Date) },
    });
  ...
  test("should use custom accessible names for the nav buttons", async ({ mount }) => {
    // ARRANGE
    const component = await mount(Calendar, {
      props: { month: JANUARY_2024, previousMonthLabel: "Prev", nextMonthLabel: "Next" },
    });
...
test.describe("year / month views", () => {
  ...
  test("should drill year → month → day and report the picked month", async ({ mount }) => {
    ...
    const component = await mount(Calendar, {
      props: { month: JANUARY_2024 },
      on: { "update:month": (value: unknown) => months.push(value as Date) },
    });
  ...
  test("should disable years and months outside min/max", async ({ mount }) => {
    // ARRANGE
    const component = await mount(Calendar, {
      props: { month: JANUARY_2024, min: new Date(2024, 2, 1), max: new Date(2025, 5, 30) },
    });
  ...
  test("should swap the day grid out entirely while browsing year/month views", async ({
    ...
    const component = await mount(Calendar, {
      props: {
        mode: "range",
        month: JANUARY_2024,
        modelValue: [new Date(2024, 0, 10), new Date(2024, 0, 14)],
      } as never,
    });
...
test.describe("range mode", () => {
  ...
  test("should paint adjacent-month days in range without making them an end", async ({
    ...
    const component = await mount(Calendar, {
      props: {
        mode: "range",
        month: JANUARY_2024,
        modelValue: [new Date(2024, 0, 29), new Date(2024, 1, 8)],
      } as never,
    });
  ...
  test("should commit on the second click and order the pair", async ({ mount }) => {
    ...
    // ARRANGE
    const component = await mount(Calendar, {
      props: { mode: "range", month: JANUARY_2024 } as never,
      on: { "update:modelValue": (value: unknown) => picked.push(value as [Date, Date]) },
    });
  ...
  test("should start a fresh range on the click after a committed one", async ({ mount }) => {
    ...
    await component.update({
      props: {
        mode: "range",
        month: JANUARY_2024,
        modelValue: [new Date(2024, 0, 20), new Date(2024, 0, 25)],
      } as never,
    });
