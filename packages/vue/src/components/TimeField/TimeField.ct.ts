...
test("should render with the default classes and no modifiers", async ({ mount }) => {
  // ARRANGE
  const component = await mount(TimeField, { slots: { label: "Time" } });
...
test("should apply the error modifier", async ({ mount }) => {
  // ARRANGE
  const component = await mount(TimeField, {
    props: { error: true },
    slots: { label: "Time", "helper-text": "Required" },
  });
...
test("should report a Date once a full time is typed", async ({ mount }) => {
