...
import SwipeableDrawer from "./SwipeableDrawer.vue";
...
const meta: Meta<SwipeableDrawerProps> = {
  ...
  render: (args) => ({
    components: { SwipeableDrawer, Button },
    setup() {
    ...
    template: `
      <div :style="surface">
        ...
        <SwipeableDrawer v-bind="args" :open="open" @open="open = true" @close="open = false">
          <div :style="panel">
...
export const Anchors: Story = {
  render: () => ({
    components: { SwipeableDrawer, Button },
    setup() {
      const anchor = ref<DrawerAnchor>("left");
      const open = ref(false);
      const anchors: DrawerAnchor[] = ["left", "right", "top", "bottom"];
      function openFrom(item: DrawerAnchor) {
        anchor.value = item;
        open.value = true;
      }
    ...
    template: `
      <div :style="surface">
        ...
        <SwipeableDrawer :open="open" :anchor="anchor" @open="open = true" @close="open = false">
          <div :style="panel">
            <h2 :style="heading">anchor = {{ anchor }}</h2>
            <p style="margin: 0">Drag me back toward the {{ anchor }} edge.</p>
          </div>
        </SwipeableDrawer>
      </div>`,
  }),
