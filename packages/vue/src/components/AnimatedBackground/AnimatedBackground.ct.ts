...
test("should apply the preset modifier", async ({ mount }) => {
  // ARRANGE
  const component = await mount(AnimatedBackground, { props: { preset: "neon" } as never });
...
test("should scale the star field with quality", async ({ mount }) => {
  // ARRANGE
  const component = await mount(AnimatedBackground, { props: { quality: "low" } as never });
...
test("should drop the firework layer when disabled", async ({ mount }) => {
  // ARRANGE
  const component = await mount(AnimatedBackground, { props: { fireworks: true } as never });
  ...
  // ACT
  await component.update({ props: { fireworks: false } as never });
...
test("should opt out of reduced motion with a modifier", async ({ mount }) => {
  // ARRANGE
  const component = await mount(AnimatedBackground, {
    props: { respectReducedMotion: false } as never,
  });
...
test("should keep the star field stable between renders", async ({ mount }) => {
  ...
  // Stars are laid out after the initial mount, so wait for one to actually
