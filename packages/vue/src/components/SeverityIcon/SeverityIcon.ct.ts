...
test.describe("Screenshot tests", () => {
  executeMatrixScreenshotTest({
    ...
    args: (column, row) => ({
      props: { severity: column, shape: row } as never,
    }),
  });
  ...
  executeMatrixScreenshotTest({
    ...
    args: (column, row) => ({
      ...
      slots: row === "custom-icon" ? { default: MOCK_PLAYWRIGHT_ICON } : undefined,
    }),
  });
});
...
test("should apply a severity modifier only for non-info severities", async ({ mount }) => {
  // ARRANGE
  const component = await mount(SeverityIcon, { props: { severity: "success" } as never });
...
test("should apply size modifiers only for non-medium sizes", async ({ mount }) => {
  // ARRANGE
  const component = await mount(SeverityIcon, { props: { size: "small" } as never });

  // ASSERT
  await expect(component).toHaveClass(/okkly-severity-icon--small/);

  // ACT
  await component.update({ props: { size: "medium" } as never });
...
test("should apply the rounded shape modifier", async ({ mount }) => {
  // ARRANGE
  const component = await mount(SeverityIcon, { props: { shape: "rounded" } as never });
...
test("should render a custom icon override", async ({ mount }) => {
  // ARRANGE
  const component = await mount(SeverityIcon, {
    slots: { default: `<span class="glyph">${MOCK_PLAYWRIGHT_ICON}</span>` },
  });
...
test("should pick a distinct default glyph per severity", async ({ mount }) => {
  ...
  // ACT
  await component.update({ props: { severity: "danger" } as never });
