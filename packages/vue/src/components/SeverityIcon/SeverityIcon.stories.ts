...
import type { SeverityIconProps, SeverityIconSeverity } from "./SeverityIcon.types";
 ...
 */
const meta: Meta<SeverityIconProps> = {
  ...
  render: (args) => ({
    ...
    setup: () => ({ args, surface }),
    template: `<div :style="surface"><SeverityIcon v-bind="args" /></div>`,
  }),
};

export default meta;
type Story = StoryObj<SeverityIconProps>;
  ...
  render: () => ({
    ...
    setup: () => ({
      ...
      severities: [
        ...
        "primary",
        "neutral",
      ] as SeverityIconSeverity[],
    }),
    template: `
      <div :style="{ ...surface, flexWrap: 'wrap', gap: '24px' }">
        <div v-for="severity in severities" :key="severity" style="display: grid; justify-items: center; gap: 10px">
  ...
  render: () => ({
    ...
    template: `
      <div style="width: 460px; padding: 8px; border-radius: 14px; border: 1px solid var(--okkly-border-subtle); background: var(--okkly-bg-surface); font-family: var(--okkly-font-family-sans)">
        <div v-for="row in rows" :key="row.title" style="display: flex; align-items: center; gap: 12px; padding: 10px 12px">
  ...
  render: () => ({
    ...
    template: `
      <div style="display: grid; justify-items: center; gap: 12px; width: 360px; padding: 26px 24px; border-radius: 16px; border: 1px solid var(--okkly-border-subtle); background: var(--okkly-bg-surface); text-align: center; font-family: var(--okkly-font-family-sans)">
        <SeverityIcon severity="danger" size="large"><span v-html="iconTrash" /></SeverityIcon>
        <strong style="font-size: var(--okkly-font-size-lg)">Delete this project?</strong>
        <p :style="\`margin: 0; \${captionStyle}\`">Night drive vol. 2 and its 12 tracks will be removed. This can't be undone.</p>
      </div>`,
  }),
};
...
export const SizesAndShapes: Story = {
  ...
  render: () => ({
    components: { SeverityIcon },
    setup: () => ({ surface, sizes: ["small", "medium", "large"] }),
    template: `
      <div :style="{ ...surface, gap: '32px' }">
        <div :style="{ ...surface, gap: '14px' }">
          <SeverityIcon v-for="size in sizes" :key="size" severity="success" :size="size" />
        </div>
        <div :style="{ ...surface, gap: '14px' }">
          <SeverityIcon v-for="size in sizes" :key="size" severity="info" :size="size" shape="rounded" />
        </div>
      </div>`,
  }),
};
...
/**
 * The default slot replaces the default glyph while keeping the tint — how the
 * dialog above gets a bin instead of a cross.
 */
export const CustomIcon: Story = {
  name: "Custom icon",
  render: () => ({
    components: { SeverityIcon },
    setup: () => ({ surface, iconTrash, iconUpload }),
    template: `
      <div :style="surface">
        <SeverityIcon severity="primary"><span v-html="iconUpload" /></SeverityIcon>
        <SeverityIcon severity="danger"><span v-html="iconTrash" /></SeverityIcon>
        <SeverityIcon severity="neutral" shape="rounded"><span v-html="iconUpload" /></SeverityIcon>
      </div>`,
  }),
};
