...
import DateTimePickerFixture from "../../playwright/fixtures/DateTimePickerFixture.vue";
...
test.describe("Screenshot tests", () => {
  executeMatrixScreenshotTest({
    ...
    args: (column, row) => ({
      props: { color: column, format: row as "24h" | "12h", defaultValue: NOV_8_2024 },
    }),
  });
  ...
  test.describe("frozen clock", () => {
    ...
    executeMatrixScreenshotTest({
      ...
      component: DateTimePicker,
      args: (column) => ({
        props: { defaultValue: column === "empty" ? null : NOV_8_2024 },
        slots: column === "with-timezone" ? { "timezone-label": "GMT+2" } : undefined,
      }),
    });
  });
});
...
test("should render a Calendar and a TimePicker with no date selected", async ({ mount }) => {
  // ARRANGE
  const component = await mount(DateTimePicker, { props: { defaultValue: null } });
...
test("should share its color with the calendar, the wheels and Confirm", async ({ mount }) => {
  // ARRANGE
  const component = await mount(DateTimePicker, { props: { color: "dante" } });
  ...
  expect(
    await component
      ...
      .evaluate((element) =>
        (element as HTMLElement).style.getPropertyValue("--okkly-calendar-tone"),
      ),
...
test("should show the formatted summary once a day is selected", async ({ mount }) => {
  // ARRANGE
  const component = await mount(DateTimePicker, {
    props: { defaultValue: new Date(2024, 10, 8, 0, 0) },
  });
...
test("should combine a picked day with the wheel time and fire update:modelValue", async ({
  mount,
}) => {
  const changes: Date[] = [];
  ...
  const component = await mount(DateTimePicker, {
    ...
    on: { "update:modelValue": (value: Date) => changes.push(value) },
  });
...
test("should disable Confirm until a day is selected, then report the value", async ({ mount }) => {
  ...
  const component = await mount(DateTimePicker, {
    props: { defaultValue: null },
    on: { confirm: (value: Date) => confirmed.push(value) },
  });
...
test("should render no timezone chip when the timezone-label slot is unfilled", async ({
  mount,
}) => {
  // ARRANGE
  const component = await mount(DateTimePicker);
...
test("should render the timezone chip when the timezone-label slot is filled", async ({
  mount,
}) => {
  // ARRANGE — its own mount rather than `mount` + `component.update({ slots
  // })`: Playwright's Vue CT harness doesn't reliably add a slot on update
  // that wasn't present at the initial mount (its own docs note slots reset
  // on update unless re-passed; introducing one from nothing goes further and
  // just doesn't render), the same class of harness limitation Calendar's
  // style-clearing test hit from a different angle.
  const component = await mount(DateTimePicker, {
    slots: { "timezone-label": "GMT+2" },
  });
...
test("should stay fully controlled when modelValue is supplied", async ({ mount }) => {
  const changes: Date[] = [];

  // ARRANGE — DateTimePickerFixture binds `:model-value`/`@update:model-value`
  // together in a real template so the fixed prop genuinely wins; see its
  // doc comment for why mounting DateTimePicker directly can't exercise this.
  const component = await mount(DateTimePickerFixture, {
    props: { modelValue: new Date(2024, 10, 8, 0, 0) },
    on: { change: (value: Date) => changes.push(value) },
  });
  ...
  // ACT
  await component.update({ props: { modelValue: new Date(2024, 10, 15, 0, 0) } });
...
test("should format the summary to match the format prop", async ({ mount, page }) => {
  ...
  // ARRANGE
  const component = await mount(DateTimePicker, {
    props: { defaultValue: new Date(2024, 10, 8, 13, 0), format: "24h" },
    on: { "update:modelValue": (value: Date) => changes.push(value) },
  });
  ...
  await component.update({ props: { defaultValue: new Date(2024, 10, 8, 13, 0), format: "12h" } });
  const hours = component.getByRole("spinbutton", { name: "Hours" });
...
test("should use custom slot content for the summary, empty state and confirm button", async ({
  mount,
}) => {
  // ARRANGE
  const component = await mount(DateTimePicker, {
    props: { defaultValue: null },
    slots: {
      "summary-label": "Time slot",
      "empty-label": "Pick a day",
      "confirm-label": "Book it",
    },
  });
...
test("should render no shortcut chips column", async ({ mount }) => {
  // ARRANGE
  const component = await mount(DateTimePicker, { props: { defaultValue: NOV_8_2024 } });
