...
import InlineActionFixture from "../../playwright/fixtures/InlineActionFixture.vue";
import type {
  InlineActionColor,
  InlineActionFill,
  InlineActionSize,
...
test.describe("Screenshot tests", () => {
  executeMatrixScreenshotTest({
    ...
    args: (column, row) => ({
      props: { fill: column, size: row, modelValue: "a@b.dev" } as never,
    }),
  });
...
test("should render the input and the action button", async ({ mount }) => {
  // ARRANGE
  const component = await mount(InlineAction, { props: { placeholder: "you@company.com" } });
...
test("should fire update:modelValue when typing", async ({ mount }) => {
  let changes = 0;

  // ARRANGE
  const component = await mount(InlineAction, {
    on: { "update:modelValue": () => (changes += 1) },
  });
...
test("should fire action when the button is clicked", async ({ mount }) => {
  let actions = 0;

  // ARRANGE — InlineActionFixture forwards `@action` out as `trigger` from a
  // real template; see its doc comment for why mounting InlineAction
  // directly with `on: { action }` can't be used here.
  const component = await mount(InlineActionFixture, {
    props: { modelValue: "a@b.dev" } as never,
    on: { trigger: () => (actions += 1) },
  });
...
test.describe("effective state priority (disabled > loading > readonly > state)", () => {
  test("should let disabled win over an explicit state prop", async ({ mount }) => {
    // ARRANGE
    const component = await mount(InlineAction, {
      props: { modelValue: "a@b.dev", state: "success", disabled: true } as never,
    });
  ...
  test("should show a spinner instead of the action icon while loading", async ({ mount }) => {
    // ARRANGE
    const component = await mount(InlineAction, {
      props: { modelValue: "a@b.dev", loading: true, action: "Sending…" } as never,
    });
  ...
  test("should lock the button and mark the input read-only", async ({ mount }) => {
    // ARRANGE
    const component = await mount(InlineAction, {
      props: { modelValue: "a@b.dev", readonly: true } as never,
    });
  ...
  test("should not fire action while read-only", async ({ mount }) => {
    let actions = 0;

    // ARRANGE — same fixture, same reason as "should fire action…" above.
    const component = await mount(InlineActionFixture, {
      props: { modelValue: "a@b.dev", readonly: true } as never,
      on: { trigger: () => (actions += 1) },
    });
...
test("should show the message caption only when provided, tinted by state", async ({ mount }) => {
  // ARRANGE
  const component = await mount(InlineAction, { props: { modelValue: "a@b.dev" } as never });
...
test("should apply a color modifier only when color is explicitly set", async ({ mount }) => {
  ...
  // ACT
  await component.update({ props: { color: "dante" } });
...
test("should apply a fill modifier only for non-filled fills", async ({ mount }) => {
  // ARRANGE
  const component = await mount(InlineAction, {
    props: { modelValue: "a@b.dev", fill: "glass" } as never,
  });
  ...
  // ACT
  await component.update({ props: { fill: "filled" } });
...
test("should apply a size modifier only for non-medium sizes", async ({ mount }) => {
  // ARRANGE
  const component = await mount(InlineAction, {
    props: { modelValue: "a@b.dev", size: "small" } as never,
  });
