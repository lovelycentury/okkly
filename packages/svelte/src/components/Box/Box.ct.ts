...
import BoxFixture from "../../playwright/fixtures/BoxFixture.svelte";
...
test("should render a div with the base classes and nothing else", async ({ mount }) => {
  // ARRANGE
  const component = await mount(Box, { slots: { children: "Content" } });
...
test("should merge a consumer's class and style after its own", async ({ mount }) => {
  // ARRANGE
  const component = await mount(Box, {
    props: { p: 2, class: "custom", style: "opacity: 0.5" },
  });
...
test.describe("spacing", () => {
  ...
  test("should rescale numeric steps when the spacing unit is overridden", async ({ mount }) => {
    // ARRANGE — a consumer's `style` comes after the system-prop variables
    const component = await mount(Box, {
      props: { p: 2, style: "--okkly-space-unit: 0.5rem" },
    });
...
test.describe("colors", () => {
  ...
  test("should draw a numeric border solid, and let borderColor recolor it", async ({ mount }) => {
    // ARRANGE — mounted with `borderColor` at the default token: the Svelte
    // mount harness updates the props a component was mounted with, not new ones
    const component = await mount(Box, { props: { border: 2, borderColor: "border.default" } });
