...
/**
 * `@playwright/experimental-ct-svelte` trails the workspace's pinned
 * `@playwright/test` (see `pnpm-workspace.yaml`'s `overrides` comment), so
 * this package pins its own, older `@playwright/test` + `playwright-core`
 * directly in `package.json` instead of `catalog:`. That makes this an
 * intentionally separate Playwright/browser toolchain from `@okkly/react`'s —
 * CI runs it in its own container image pinned to the same version
 * (`.github/workflows/svelte-playwright.yml`).
 */
 ...
 * it. Each component owns one `*.ct.ts` next to its source.
 *
...
const getDefaultConfig = (options?: DefineOkklyPlaywrightConfigOptions) => {
  const config = {
    ...
    use: {
      ...
      // React takes 3100, Angular 3101 and Vue 3102, so every package's suite
      // can run at once.
      ctPort: 3103,
      ctViteConfig: {
        plugins: [svelte()],
        resolve: {
          alias: {
            ...
            "@okkly/svelte": fileURLToPath(new URL("./src/index.ts", import.meta.url)),
          },
        },
      },
    },
