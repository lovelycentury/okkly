...
import { Box } from "./Box";
import type { BoxSystemProps } from "./Box.types";
...
/**
 * Renders a shared test layout: a Box, an optional Box inside it, and the
 * swatches. A plain function rather than a component: Playwright can only
 * mount components imported from their own modules, never ones declared in a
 * test.
 */
const renderTree = ({ outer, inner, swatches }: BoxTree) => {
  const content = swatches.map((color, index) => <Box key={index} {...boxSwatch(color)} />);
  return <Box {...outer}>{inner ? <Box {...inner}>{content}</Box> : content}</Box>;
};
...
test("should keep system props off the element's attributes", async ({ mount }) => {
  // ARRANGE
  const component = await mount(
    <Box p={2} color="text.primary" width={10} display="flex" container data-testid="box" />,
  );
...
test.describe("responsive values", () => {
  test("should use the base value below the breakpoint", async ({ mount, page }) => {
    ...
    const component = await mount(<Box {...BOX_RESPONSIVE_LAYOUT} />);
...
test.describe("container queries", () => {
  ...
  test("should use the base value in a container narrower than the key", async ({ mount }) => {
    // ARRANGE
    const component = await mount(renderTree(boxInContainer(400)));
  ...
  test("should switch at the container's width, whatever the viewport", async ({ mount, page }) => {
    ...
    await page.setViewportSize({ width: 600, height: 400 });
    const component = await mount(renderTree(boxInContainer(700)));
  ...
  test("should let a container value win over a viewport value", async ({ mount, page }) => {
    ...
    const component = await mount(renderTree(boxInContainer(500)));
