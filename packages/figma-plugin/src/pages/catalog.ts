...
async function header(
  ...
  const gCol = async (mark: string, color: string, text: string) => {
    ...
    v.appendChild(await makeText(t, "body/sm", text, "text/secondary", { maxWidth: textMax }));
    c.appendChild(v);
    return c;
  };
...
async function tileGrid(
  t: ThemeContext,
  tiles: Array<{ label: string; node: SceneNode }>,
  tileW: number,
): Promise<FrameNode> {
  const wrap = canvas(t);
  const GAP = 24;
  ...
  for (let i = 0; i < tiles.length; i += perRow) {
    ...
    for (const tile of slice) {
      ...
      cap.appendChild(
        await makeText(t, "mono/sm", tile.label, "text/secondary", { maxWidth: tileW }),
      );
      col.appendChild(cap);
      row.appendChild(col);
    }
...
async function propsTable(
  ...
  for (const row of rows) {
    ...
    nc.appendChild(await makeText(t, "body/sm", row.note, "text/secondary", { maxWidth: wNote }));
    r.appendChild(pc);
    r.appendChild(tc);
    r.appendChild(dc);
    r.appendChild(nc);
...
async function buttonBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const btnTones: Array<{ label: string; node: SceneNode }> = [];
  for (const [en, tk] of TONES_ACCENT)
    btnTones.push({
      label: en,
  ...
  const props: PropRow[] = [
    ...
    },
    {
      ...
      type: "token",
      def: "accent/primary",
      note: "Primary fill colour (dante-ready).",
    },
    {
      prop: "shape",
      type: "rounded|pill",
      def: "pill",
      note: "Corner style (pill = soft).",
    },
    {
      prop: "size",
      type: "sm|md|lg",
...
async function switchBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const swTones: Array<{ label: string; node: SceneNode }> = [];
  for (const [en, tk] of TONES_9)
    swTones.push({ label: en, node: await drawSwitch(t, true, false, "md", tk) });
  ...
  const props: PropRow[] = [
    ...
    },
    {
      ...
      type: "token",
      def: "accent/primary",
      note: "On-state colour (dante-ready).",
    },
...
async function textFieldBoard(t: ThemeContext): Promise<FrameNode> {
  const stateList: TfState[] = ["Default", "Focus", "Filled", "Error", "Disabled"];
  const states = await tileGrid(
    t,
    await Promise.all(stateList.map(async (s) => ({ label: s, node: await drawTextField(t, s) }))),
    320,
  );
  const sizes = await tileGrid(
    t,
  ...
  const props: PropRow[] = [
    ...
    { prop: "disabled", type: "boolean", def: "false", note: "Blocks input." },
  ];
  return componentBoard(
    t,
    "TextField",
    "Short free-form text",
    "Known options → use Select",
    [
...
async function checkboxBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const cbTones: Array<{ label: string; node: SceneNode }> = [];
  for (const [en, tk] of TONES_9)
    cbTones.push({ label: en, node: await drawCheckbox(t, "Checked", false, "md", tk) });
  ...
  const groups = await tileGrid(
    ...
    [
      ...
      { label: "Nested (select all)", node: nestedCol },
    ],
    280,
  );
  ...
  const props: PropRow[] = [
    ...
    },
    {
      ...
      type: "Checkbox[]",
      def: "—",
      note: "Nested options (select all).",
    },
    {
      prop: "onChange",
      type: "(e,checked)=>void",
...
async function inputActionBoard(t: ThemeContext): Promise<FrameNode> {
  const stateList: Array<[IaState, string?]> = [
    ["Default"],
    ["Hover"],
    ["Focus"],
    ["Filled"],
  ...
  const toneList: Array<[string, string]> = [
  ...
  const fillList: Array<[IaFill, string]> = [
    ["filled", "Filled"],
  ...
  for (const [f, label] of fillList) {
    ...
    group.appendChild(await makeText(t, "label/sm", label, "text/secondary"));
  ...
  return componentBoard(
    t,
    "Input — Inline Action",
    "Value + one inline action (copy, subscribe, send)",
    "Several actions or a real form → TextField + Button",
    [
      await block(t, "States", states),
      await block(t, "Tones", tones),
...
async function richEditorBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  for (const head of ["Toolbar", "Content", "Footer"]) {
  ...
  return componentBoard(
    ...
    "Plain notes or a single value → Text Area",
    [
...
async function dateFieldBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    ...
    "Free calendar browsing → use Calendar",
...
async function autocompleteBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const props: PropRow[] = [
    ...
    },
    {
      ...
      type: "(q)=>void",
      def: "—",
      note: "Query as you type (async ok).",
    },
    {
      prop: "renderOption",
      type: "(o)=>ReactNode",
  ...
  const props: PropRow[] = [
    {
      ...
      type: "string",
      def: "—",
      note: "Source (transparent PNG best).",
    },
    {
      prop: "style",
    ...
    },
    {
      ...
      type: "token",
      def: "xl",
      note: "Corners (ignored if cutout).",
    },
  ];
  return componentBoard(
    t,
    "Media — Photo",
    "Portraits & hero cutouts on a dark background",
    "Icons or logos → use an Icon / SVG",
...
async function linkCardBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const props: PropRow[] = [
    ...
    { prop: "href", type: "string", def: "—", note: "Destination URL." },
    { prop: "onClick", type: "(e)=>void", def: "—", note: "Click handler." },
  ];
  return componentBoard(
    t,
    ...
    "Long, browsable lists → use List / Table",
    [
...
async function statBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    ...
    "Comparisons or trends over time → use a Chart",
    [
...
async function calendarBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    ...
    "Ranges / time / scheduling → use a DateRange picker",
...
async function carouselBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    t,
    "Carousel",
    "Swipe through a few visual slides (Embla)",
    "Long scrollable lists → use a List / Grid",
...
async function timePickerBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    t,
    "TimePicker",
    "Pick hours & minutes, iOS wheel style",
...
async function dateTimeBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    ...
    "Only a date → use Calendar; only time → TimePicker",
  ...
  const props: PropRow[] = [
    {
      ...
      type: "string",
      def: "—",
      note: "Background image (transparent OK).",
    },
    {
      prop: "logo",
      type: "ReactNode",
...
async function chipBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    ...
    "A committing action → use Button",
    [
...
async function radioBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const rdTones: Array<{ label: string; node: SceneNode }> = [];
  for (const [en, tk] of TONES_ACCENT)
    rdTones.push({ label: en, node: await drawRadio(t, true, false, "md", tk) });
...
async function tabsBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const props: PropRow[] = [
    ...
    },
    {
      ...
      type: "token",
      def: "accent/primary",
      note: "Active tab colour (dante-ready).",
    },
    {
      prop: "onChange",
      type: "(i)=>void",
  ...
  return componentBoard(
    ...
    "Navigate to a new page → use Links/Nav",
  ...
  const props: PropRow[] = [
    {
      ...
      def: "info",
      note: "Severity & color; dante = announcements.",
    },
...
async function avatarBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    ...
    "Decorative imagery → use Media / Photo",
    [
      await block(t, "Sizes", sizes),
...
async function selectBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    ...
    "Free text → use TextField / Autocomplete",
    [
      await block(t, "States", states),
      await block(t, "Sizes", sizes),
      await block(t, "Variants", variants),
...
async function progressBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const circular = await tileGrid(
    t,
    [
      { label: "70%", node: await drawProgressCircular(t, 0.7, 56) },
      { label: "40%", node: await drawProgressCircular(t, 0.4, 56) },
    ],
    120,
  );
  const pgLin: Array<{ label: string; node: SceneNode }> = [];
  for (const [en, tk] of TONES_9)
    pgLin.push({ label: en, node: drawProgressLinear(t, 0.6, false, 240, tk) });
  const pgCirc: Array<{ label: string; node: SceneNode }> = [];
  for (const [en, tk] of TONES_9)
    pgCirc.push({ label: en, node: await drawProgressCircular(t, 0.7, 56, tk) });
  ...
  return componentBoard(
    ...
    "Placeholder while loading → use Skeleton",
    [
...
async function paginationBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    ...
    "Infinite feed → use a Load-more / scroll",
    [
...
async function chipGroupBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const showcase = await tileGrid(
    ...
    [
      ...
      { label: "Overflow +N", node: overflow },
      { label: "Files", node: files },
      { label: "In a field (TokenField)", node: field },
    ],
    360,
  );
  const props: PropRow[] = [
    ...
    },
    {
      ...
      type: "boolean",
      def: "false",
      note: "Wrap in an input (TokenField).",
    },
...
const TONES_3: Array<[string, string]> = [
  ["Mint", "accent/primary"],
  ["Dante", "accent/dante"],
  ["Indigo", "accent/secondary"],
];
/** All six accents — the palette a section can be tinted with. */
const TONES_ACCENT: Array<[string, string]> = [
  ...TONES_3,
  ["Violet", "accent/violet"],
  ["Ember", "accent/ember"],
  ["Ice", "accent/ice"],
];
const TONES_FEEDBACK: Array<[string, string]> = [
  ["Success", "feedback/success"],
  ["Warning", "feedback/warning"],
  ["Danger", "feedback/danger"],
];
/** Accents + feedback — the full row, used wherever tone is a real axis. */
const TONES_9: Array<[string, string]> = [...TONES_ACCENT, ...TONES_FEEDBACK];
...
async function contextMenuBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const props: PropRow[] = [
    ...
    },
    {
      ...
      type: "boolean",
      def: "false",
      note: "Danger styling (e.g. Delete).",
    },
    {
      prop: "align / side",
  ...
  return componentBoard(
    ...
    "A primary page action → use Button/Menu",
...
async function dividerBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const props: PropRow[] = [
    ...
    },
    {
      ...
      type: "ReactNode",
      def: "—",
      note: "Optional centered label (e.g. OR).",
    },
    {
      prop: "flexItem",
      type: "boolean",
      def: "false",
      note: "Stretch inside a flex row.",
    },
  ];
  return componentBoard(
    t,
...
async function badgeBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const toneTiles: Array<{ label: string; node: SceneNode }> = [];
  for (const [en, tk] of tones)
    toneTiles.push({
      label: en,
  ...
  const el: Array<{ label: string; node: SceneNode }> = [
    {
      label: "Count",
...
async function skeletonBoard(t: ThemeContext): Promise<FrameNode> {
  const variants: Array<{ label: string; node: SceneNode }> = [
  ...
  const anim: Array<{ label: string; node: SceneNode }> = [
...
async function snackbarBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    ...
    "Critical/blocking info → use a Dialog",
...
async function tableBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    ...
    "A few key-values → use a description List",
    [
...
async function kbdBoard(t: ThemeContext): Promise<FrameNode> {
  const keys = await tileGrid(
    t,
    [
      { label: "Cmd", node: await drawShortcut(t, ["⌘"]) },
      { label: "Shift", node: await drawShortcut(t, ["⇧"]) },
      { label: "Ctrl", node: await drawShortcut(t, ["⌃"]) },
      { label: "Option", node: await drawShortcut(t, ["⌥"]) },
      { label: "Esc", node: await drawShortcut(t, ["Esc"]) },
      { label: "Enter", node: await drawShortcut(t, ["↵"]) },
  ...
  const props: PropRow[] = [
    {
      ...
      def: "ring",
      note: "Comet adds a glowing tail.",
    },
    {
      prop: "size",
      type: "sm | md | lg",
    ...
    },
    {
      ...
      type: "token",
      def: "accent/primary",
      note: "Any accent or feedback token.",
    },
    {
      prop: "track",
      type: "boolean",
    ...
    },
    {
      ...
      type: "string",
      def: "—",
      note: "Accessible text next to it.",
    },
  ];
  return componentBoard(
    t,
...
async function emptyStateBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    ...
    "Loading placeholder → use Skeleton",
...
async function popoverBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  return componentBoard(
    ...
    "Plain hint → Tooltip; blocking flow → Dialog",
    [
...
async function cardBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const props: PropRow[] = [
    {
      ...
      def: "solid",
      note: "Surface treatment; aura = toned glow.",
    },
    {
      prop: "media",
      type: "ReactNode",
  ...
  return componentBoard(
    ...
    "Row-like items → List; metrics → Stat card",
    [
      await block(t, "Variants", variants),
      await block(t, "Anatomy", anatomy),
...
async function stepperBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const props: PropRow[] = [
    ...
    },
    {
      ...
      type: "token",
      def: "accent/primary",
      note: "Dots & connectors colour (dante-ready).",
    },
    {
      prop: "error",
      type: "number",
  ...
  return componentBoard(
    ...
    "Page navigation → Tabs; single task progress → Progress",
    [
...
async function buttonGroupBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const props: PropRow[] = [
    ...
    },
    {
      ...
      type: "token",
      def: "accent/primary",
      note: "Fill / active colour (dante-ready).",
    },
    {
      prop: "disabled",
      type: "boolean",
  ...
  return componentBoard(
    t,
    "ButtonGroup / Split",
    "Related actions side by side; split = default + menu",
...
async function ratingBoard(t: ThemeContext): Promise<FrameNode> {
  const values = await tileGrid(
    ...
    [
      ...
      { label: "4.0", node: await drawRating(t, { value: 4 }) },
...
type FileKind = { key: string; label: string; icon: string; tone: string; ext: string };
...
async function drawFileTypeLegend(t: ThemeContext, w: number): Promise<FrameNode> {
  ...
  for (const k of FILE_TYPES) {
    ...
    const col = autoFrame({ direction: "VERTICAL", gap: 3 });
    col.appendChild(await makeText(t, "label/sm", k.label, "text/primary"));
...
async function fileUploadBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const props: PropRow[] = [
    ...
    },
    {
      ...
      type: "boolean",
      def: "true",
      note: "false → single-file variant.",
    },
    {
      prop: "variant",
      type: "zone|inline|cell|button",
      def: "zone",
      note: "Form factor; cell fits tables.",
    },
    {
      prop: "cellState",
      type: "empty|active|filled|multi|uploading|disabled",
      def: "empty",
      note: "Cell state; multi = 2+ files (rare).",
    },
    {
      prop: "stackMax",
      type: "number",
    ...
    },
    {
      ...
      type: "boolean",
      def: "true",
      note: "Speed and ETA under the bar.",
    },
    {
      prop: "tone",
      type: "token",
      def: "accent/primary",
      note: "Drag-over colour (dante-ready).",
    },
    {
      prop: "onDrop",
      type: "(files)=>void",
  ...
  return componentBoard(
    t,
    "File upload",
    "Bring files in: drag & drop + progress",
    "No progress to show at all → simple Button",
    [
...
async function avatarGroupBoard(t: ThemeContext): Promise<FrameNode> {
  ...
  const props: PropRow[] = [
    { prop: "max", type: "number", def: "5", note: "Visible before +N." },
    {
      prop: "total",
      type: "number",
      def: "—",
      note: "Real count for the +N chip.",
    },
  ...
  return componentBoard(
    t,
    "Avatar group",
    "Show a team compactly; extras collapse to +N",
