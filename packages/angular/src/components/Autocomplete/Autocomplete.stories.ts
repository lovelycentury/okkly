...
import { OkklyAutocomplete } from "./Autocomplete";
import type { AutocompleteColor, AutocompleteOption, AutocompleteSize } from "./Autocomplete";
...
const TEAM_OPTIONS: AutocompleteOption[] = [
...
const CITY_OPTIONS: AutocompleteOption[] = [
  ...
  { value: "kyiv", label: "Kyiv", description: "Ukraine" },
...
/**
 * Filter-as-you-type field, with free-text and multi-select tag support.
 */
const meta: Meta<AutocompleteArgs> = {
  title: "Control/Autocomplete",
  component: OkklyAutocomplete,
  decorators: [moduleMetadata({ imports: [OkklyAutocomplete] })],
  args: {
  ...
  render: (args) => ({ props: args, template: `<okkly-autocomplete${bindings} />` }),
};
 ...
 */
export const Filled: Story = { args: { value: "engineering", inputValue: "Engineering" } };
...
export const Disabled: Story = {
  args: { disabled: true, value: "engineering", inputValue: "Engineering" },
};
...
/**
 * This example shows free text committed as a value even when it matches no option.
 */
export const FreeSolo: Story = { args: { freeSolo: true } };

/**
 * This example shows grouped options under sticky headers, reordered so each
 * group's members are contiguous, plus a per-option description.
 */
export const Grouped: Story = {
  render: () => ({
    props: {
      options: CITY_OPTIONS,
      groupBy: (option: AutocompleteOption) => CITY_REGION[option.value] ?? "Other",
    },
    template: `<okkly-autocomplete label="City" placeholder="Search a city…" [options]="options" [groupBy]="groupBy" />`,
  }),
};
...
export const WiderPopup: Story = {
  ...
  render: (args) => ({
    props: args,
    template: `<okkly-autocomplete${bindings} [popupWidth]="320" />`,
  }),
};
    ...
    template: `
      <div style="display: flex; flex-direction: column; gap: 16px">
        @for (size of sizes; track size) {
          <okkly-autocomplete [size]="size" label="Team" [options]="options" value="engineering" />
        }
      </div>`,
  }),
};
...
/**
 * This example shows an autocomplete whose value lives entirely in the parent.
 */
  ...
  render: () => ({
    ...
    template: `
      <div style="display: flex; flex-direction: column; gap: 12px; align-items: flex-start">
        <okkly-autocomplete label="Team" [options]="options" [(value)]="team" />
  ...
  render: () => ({
    ...
    template: `
      <form method="get" style="display: flex; flex-direction: column; gap: 12px; align-items: flex-start">
        <okkly-autocomplete label="Team" name="team" [options]="options" value="engineering" />
...
/**
 * This example overrides the field's own `--okkly-autocomplete-*` variables inline.
 */
  ...
  render: () => ({
    ...
    template: `
      <okkly-autocomplete
        ...
        [options]="options"
        value="engineering"
        style="--okkly-autocomplete-border-color: #d946ef; --okkly-autocomplete-focus-border-color: #d946ef"
      />`,
  }),
};
